...
# Q. Assess normality of horsepower values.
# Kurtosis
skewness = df['horsepower'].skew(axis = 0, skipna = True)
print("Skewness: ", skewness)
# Skewness
kurtosis = df['horsepower'].kurtosis()
print("Kurtosis: ", kurtosis)

# (1) Visual Inspection
import seaborn as sns
import matplotlib.pyplot as plt
plt.show(sns.distplot(df['horsepower']))

# (2) NHST
from scipy import stats
shapiroHorsepower = stats.shapiro(df['horsepower'])
print("Shapiro-Wilk Test for 'horsepower': ",shapiroHorsepower)

# (3) Q-Q Plot
plt.show(stats.probplot(df['horsepower'], dist="norm", plot=plt))
